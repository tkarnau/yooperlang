...
Features land phase-by-phase per [plans/roadmap.md](plans/roadmap.md). Currently mid Phase 7. Recent landings: Phase 7.4 (trait-qualified call syntax `Trait.method(ref x, ...)` — see [plans/phase-7-4-trait-call-syntax.md](plans/phase-7-4-trait-call-syntax.md)); Phase 7.1 (user-defined generic structs/functions/traits, with `Task<T>` no longer special-cased in the parser); Phase 6.5 (`layout { align ... }`, kind composition with `&`); Phase 6.4 (`propagates`).
