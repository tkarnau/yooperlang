...
- **`propagates<K>` is a "must handle, or hand off" contract (Phase 6.4 strict).** A type that declares `propagates<K>` says: any binding of this type carries `K`'s obligation; the obligation cannot be silently dropped. The user has three legal exits:
  1. **Auto-cleanup**: declare the kind keyword on the binding (`usedisposable arr: T = ...`) and the compiler injects the cleanup call at scope end. The keyword is a *commitment to local cleanup*: it overrides transfer-via-return. If the binding is then returned from a propagating function, the cleanup still fires before `ret` (the resulting program is unsound — caller receives a disposed value — but that's the user's contradiction to resolve).
