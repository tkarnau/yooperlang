...
Phases 1 through 8 (plus library phases A–D) have landed: typechecker, structs, errors-as-values, modules + FFI, refs / arrays / control flow, traits, kinds, generics + trait bounds, trait-qualified calls, sum types + unions, the full Phase 8 networking prerequisites (unsafe_ptr, c_int aliases, buffer interop, errno, module-level state, suspension + multiplexer + timers, string/bytes/Vec), and the four library phases (std/core, std/net, std/http types + parser, std/http server). See [plans/completed/]() for the per-phase plan documents.
...
> See [plans/completed/phase-9-f-display-in-templates.md](phase-9-f-display-in-templates.md).
...
> **Status: landed.** See
> [plans/completed/phase-9-g-vtables.md](phase-9-g-vtables.md).
...
> **Status: landed.** See
> [plans/completed/phase-9-h-fallible-enum-qmark.md](phase-9-h-fallible-enum-qmark.md).
...
> **Status: landed.** See
> [plans/completed/phase-9-i-suspendable-wait.md](phase-9-i-suspendable-wait.md).
...
> **Status: landed.** See
> [plans/completed/phase-9-j-reserved-syntax-cleanups.md](phase-9-j-reserved-syntax-cleanups.md).
> Trait `extends` (single, multiple, generic), parenthesized multi-bound type
> params `<T implements (A, B)>`, and `mustNotShare acrossThreads` (statically
> rejected at task-spawn argument sites) all parse + typecheck end-to-end.
> The old "not yet supported" parse-error branches are gone; the matching
> SPEC §16 deferral notes are retired.
