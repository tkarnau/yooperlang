...
- **No `wait` inside task bodies.** A `task` function body that contains `wait` is rejected at typecheck. Outside `main` and regular functions, `wait` is fine. (The 6.3 message promised coroutine suspension in a "future phase"; that phase landed - see [async-coroutines.md](../async-coroutines.md) - and the diagnostic now points at `await` as the in-task form. The restriction on `wait` itself still stands: suspending on a `Task<T>` handle would need a task-completion wakeup that re-queues the waiter.)
